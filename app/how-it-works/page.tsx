...
export default async function HowItWorksPage({ searchParams }: Props) {
  ...
  return (
    <main className="min-h-screen bg-brand-cream flex flex-col">
      <header className="px-8 py-6 border-b border-brand-border">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-16">
        <div className="w-full max-w-2xl stagger-children">
          ...
          <h1 className="text-3xl md:text-4xl font-light text-brand-ink mb-4 leading-snug">
            {name ? `Ready, ${name}?` : "Ready to find your style?"}
          </h1>

          <p className="text-brand-stone mb-14 max-w-md leading-relaxed">
            This takes approximately 5 minutes. Your results are personalized
            to your exact choices — no one else will get quite the same result.
          </p>

          <ol className="space-y-10 mb-16">
            {STEPS.map((step) => (
              <li key={step.number} className="flex gap-6">
                <span className="text-3xl font-light text-brand-accent shrink-0 w-10 leading-none">
                  {step.number}
                </span>
                <div>
                  <h2 className="font-medium text-brand-ink mb-1">{step.title}</h2>
          ...
          <Link
            href={quizHref}
            className="inline-block px-10 py-4 rounded-lg bg-brand-accent hover:bg-brand-accent-dark text-white font-medium text-sm tracking-wide transition-colors duration-200"
          >
            Start Quiz
          </Link>
